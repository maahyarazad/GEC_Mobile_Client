...
const NewsletterPostCard: FC<Props> = ({ post }) => {
  if (post.post_type === 1) {
    ...
    return (
      <div className="post-card w-full border-1 border-gray-300 ">
        ...
        {_post.images && _post.images != null && <ImageGrid images={_post.images.split(",")} columns={3} />}
      </div>
    );
    ...
    const images = _post?.images?.toString();
          ...
          <div className=" gap-2 flex flex-row ">
            {!!_post?.images && _post.mode === `offer` && (
              <div className="w-9rem">
                <Image
                  src={config.BASE_URL+'/user/marketplace_images/'+image + "_s2.jpg"}
                  className="w-full"
                  imageStyle={{
                    width: "140px",
                    aspectRatio: "1",
