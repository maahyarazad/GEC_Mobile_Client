...
const ExpertEventDetailEdit: React.FC = () => {
  ...
  const handleGenerateMemberCode = function () {
    const firstname = getValues('firstname').trim().toUpperCase();
    const lastname = getValues('lastname').trim().substring(0, 4).toUpperCase();
  ...
  const handleGenerateBesucherCode = function () {
    ...
    const newCode = `${getValues('firstname').trim()}_${getValues('lastname').trim()}_${randomStr}`
  ...
  const leftContents = () => {
    return (
      <div className="pl-2 font-bold text-2xl">
        {isEditMode ? "Edit" : "New"} Expert Member Details
      </div>
    );
  };
